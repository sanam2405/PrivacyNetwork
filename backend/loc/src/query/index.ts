...
async function getNearbyEntities(latitude: number, longitude: number, entityType: string) {
...
async function getNearbyEntitiesWithinDMeters(latitude: number, longitude: number, D: number, entityType: string) {
    const wktPoint = coordinatesToWKT(latitude, longitude);
    const nearbyEntities = await prisma.$queryRaw`
        SELECT *
...
async function getPrivacyEntities(latitude: number, longitude: number, D: number, age:number,  college: string, gender:string, entityType: string) {
    const wktPoint = coordinatesToWKT(latitude, longitude);
...
async function main() {
    try {
        ...
        const longitude:number = 88.1260; // Example longitude
        ...
        const nearbyEntities = await getNearbyEntities(latitude, longitude, entityType);
        console.log("USERS NEARBY");
        console.log(nearbyEntities);
        
        // SELECT * FROM nearby_entities_within_d(22.40456, 88.1260, 70000, 'user')
        const nearbyEntitiesWithinDMeters = await getNearbyEntitiesWithinDMeters(latitude, longitude, threasholdDistance, entityType);
        ...
        const privacyEntities = await getPrivacyEntities(latitude, longitude, threasholdDistance, threasholdAge, targetCollege, targetGender, entityType);
        console.log(`PRIVACY ${targetGender} USERS NEARBY WITHIN ${threasholdDistance} meters and age less than ${threasholdAge} with college ${targetCollege}`);
